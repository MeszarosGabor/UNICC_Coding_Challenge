...
@responses.activate
def test_fetch_transaction_info_ram_optimized__success():
    mock_json = [
        {
            "name": "Brent Valdez",
            "email": "annagregory@example.org",
    ...
    resp = transaction_fetcher.fetch_transaction_info_ram_optimized()

    assert list(resp) == mock_json


@responses.activate
def test_fetch_transaction_info_ram_optimized__connection_error():
    responses.add(responses.GET, constants.TRANSACTION_RAW_URL, status=500)
    with pytest.raises(requests.exceptions.HTTPError):
        # we need to trigger the raise by consuming the generator
        list(transaction_fetcher.fetch_transaction_info_ram_optimized())
